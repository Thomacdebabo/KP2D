...
class KeypointDecoder(nn.Module):
    def __init__(self):
        ...
        self.featconv  = nn.Sequential(
            nn.Conv2d(self.num_ch_dec[4], self.num_ch_dec[4], kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(self.num_ch_dec[4]),
            nn.LeakyReLU(inplace=True),
            nn.ReflectionPad2d(1),
            nn.Conv2d(self.num_ch_dec[4], self.num_ch_dec[4], 3))
